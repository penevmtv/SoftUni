...
// Jump commands will follow until you receive "Love!". Every house in the neighborhood needs a certain number of hearts delivered by 
...
// For example, we are given this neighborhood: 6@6@6. Cupid is at the start and jumps with a length of 2. He will end up at index 2 and decrease
//  the needed hearts by 2: [6, 6, 4]. Next, he jumps again with a length of 2 and goes outside the neighborhood, so he goes back to the first 
...
function heartDelivery(arrOfStrings) {
    let copyArrOfStr = arrOfStrings.slice();
    let neighborhoodArr = copyArrOfStr.shift().split(`@`).map(Number);

    let lastPositionIndex = 0;

    let jumpArr = copyArrOfStr.shift();
    while (jumpArr !== `Love!`) {
        let jump = Number(jumpArr.split(` `).pop());

        if (lastPositionIndex + jump >= neighborhoodArr.length) {
            lastPositionIndex = 0;
        } else {
            lastPositionIndex += jump;
        }

        if (neighborhoodArr[lastPositionIndex] === 0) {
            console.log(`Place ${lastPositionIndex} already had Valentine's day.`)
        } else {
            neighborhoodArr[lastPositionIndex] -= 2;
            if (neighborhoodArr[lastPositionIndex] === 0) {
        ...
        jumpArr = copyArrOfStr.shift();
    }
    console.log(`Cupid's last position was ${lastPositionIndex}.`);
    let unhappyArr = neighborhoodArr.filter(x => x > 0);
    if (unhappyArr.length > 0) {
        console.log(`Cupid has failed ${unhappyArr.length} places.`);
    } else {
