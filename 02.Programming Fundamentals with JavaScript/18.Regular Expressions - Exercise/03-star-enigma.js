...
function starEnigma(inputArray) {
    ...
    let n = Number(workingArray.shift());

    for (let i = 0; i < n; i++) {

        let currentMessage = workingArray[i];
        
        let charPattern = /[star]/gi;

        let matches = currentMessage.match(charPattern);
        ...
        let resultPattern = /@(?<name>[A-Z][a-z]+)[^@\-!:>]*:(?<population>\d+)[^@\-!:>]*!(?<attackType>[AD])![^@\-!:>]*->(?<soldiers>\d+)/;
