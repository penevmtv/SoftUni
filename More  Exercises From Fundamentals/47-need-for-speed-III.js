...
function needForSpeed(inputArr) {
    let carsObj = {};
    const numOfCars = Number(inputArr[0]);

    for (let i = 1; i <= numOfCars; i++) {
        let [car, mileage, fuel] = inputArr[i].split(`|`);

        carsObj[car] = {mil: Number(mileage), fuel: Number(fuel)};
    }

    let index = numOfCars + 1;
    while (inputArr[index] !== `Stop` && index < inputArr.length) {
        let [command, ...carInfoArr] = inputArr[index].split(` : `);

        if (command === `Drive`) {
            const car = carInfoArr.shift();
            const [distance, fuel] = carInfoArr.map(Number);

            if (carsObj[car].fuel >= fuel) {
                console.log(`${car} driven for ${distance} kilometers. ${fuel} liters of fuel consumed.`);
                carsObj[car].mil += distance;
                carsObj[car].fuel -= fuel;

            } else console.log(`Not enough fuel to make that ride`);

            if (carsObj[car].mil > 100000) {
                console.log(`Time to sell the ${car}!`);
                delete carsObj[car];
            }

        } else if (command === `Refuel`) {
            const [car, fuelStr] = carInfoArr;
            const fuel = Number(fuelStr);
        
            const currentFuel = carsObj[car].fuel;
            carsObj[car].fuel = Math.min(currentFuel + fuel, 75);
            const refueled = carsObj[car].fuel - currentFuel;
            console.log(`${car} refueled with ${refueled} liters`);

        } else if (command === `Revert`) {
            const [car, kilometersStr] = carInfoArr;
            const kilometers = Number(kilometersStr);

            if (carsObj[car].mil - kilometers >= 10000) {
                carsObj[car].mil -= kilometers;
                console.log(`${car} mileage decreased by ${kilometers} kilometers`);

            } else carsObj[car].mil = 10000;
        }

        index++;
    }
    Object.entries(carsObj).forEach(([car, infoObj]) => console.log(`${car} -> Mileage: ${infoObj.mil} kms, Fuel in the tank: ${infoObj.fuel} lt.`));

    // Alternative version with for-of loop
    
    // for (const [car, infoObj] of Object.entries(carsObj)) {
    //     console.log(`${car} -> Mileage: ${infoObj.mil} kms, Fuel in the tank: ${infoObj.fuel} lt.`);
    // }
}
needForSpeed([
  '3',
