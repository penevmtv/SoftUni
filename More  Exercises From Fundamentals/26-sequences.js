...
//After storing all arrays, your program should print them back in ascending order based on their length, if two arrays have the same length, they should be printed in
 //order of being received from the input. Each array should be printed in descending order in the format "[a1, a2, a3,… an]". Check the examples below.
...
function sequences(inputArr) {
    ...
    for (const json of inputArr) {
        ...
        array.sort((a, b) => b - a);
        const sortedJson = JSON.stringify(array);
        jsonSet.add(sortedJson);
    }   
    
    const result = Array.from(jsonSet).map(json => JSON.parse(json)).sort((a, b) => a.length - b.length);
