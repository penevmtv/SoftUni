...
function counterStrike(inputArr) {
    let initialEnergy = Number(inputArr[0]);
    let winCounter = 0;

    let index = 1;
    while (inputArr[index] !== `End of battle` && index < inputArr.length) {
        const neededEnergy = Number(inputArr[index]);

        if (initialEnergy >= neededEnergy) {
            initialEnergy -= neededEnergy;
            winCounter ++;
            
            if (winCounter % 3 === 0) initialEnergy += winCounter;
