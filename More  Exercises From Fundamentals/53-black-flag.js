...
function blackFlag(inputArr) {
    const [days, dailyPlunder, expectedPlunder] = inputArr.map(Number);
    let plunderCounter = 0;
    
    for (let i = 1; i <= days; i++) {
        plunderCounter += dailyPlunder;

        if (i % 3 === 0) plunderCounter += dailyPlunder * 0.5;

        if (i % 5 === 0) plunderCounter *= 0.7;
    }
    ...
    if (plunderCounter >= expectedPlunder) {
        ...
        let percentage = (plunderCounter / expectedPlunder) * 100;
