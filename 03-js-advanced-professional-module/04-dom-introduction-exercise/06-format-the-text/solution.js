...
// Generate HTML paragraphs as a string (Use interpolation string to create paragraph element: `<p> {text} </p>`)
//  and append it to the div with an id = "output".
...
function solve() {
  ...
  const sentencesArr = input.split('.').filter(el => el.trim().length > 0);
  
  for (let i = 0; i < sentencesArr.length; i += 3) {
    const result = sentencesArr.slice(i, i + 3).join('. ') + '.';
