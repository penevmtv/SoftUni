...
function carFactory(orderObj) {
    ...
    function engineSelecting(neededPower) {
        const typesEnginesObj = {
            smallEngine: { power: 90, volume: 1800 },
            normalEngine: { power: 120, volume: 2400 },
            monsterEngine: { power: 200, volume: 3500 }
        }
        if (neededPower <= 90) return typesEnginesObj.smallEngine;
        if (neededPower <=120) return typesEnginesObj.normalEngine;
        return typesEnginesObj.monsterEngine;
    }

    function carriageSelecting(type, color) {
        if (type === `hatchback` || type === `coupe`) return { type, color };
    }

    function wheelsSelecting(size) {
        const oddNumber = size % 2 !== 0 ? size : size - 1;

        return new Array(4).fill(oddNumber);
    }

    const carObj = {};

    carObj.model = orderObj.model;
    carObj.engine = engineSelecting(orderObj.power);
    carObj.carriage = carriageSelecting(orderObj.carriage, orderObj.color);
    carObj.wheels = wheelsSelecting(orderObj.wheelsize);
