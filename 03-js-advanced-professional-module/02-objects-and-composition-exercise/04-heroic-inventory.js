...
function heroicInventory(arrOfStrs) {
 
  function addToResultArr(name, level, items) {
    const registerObj = {
      name,
      level,
      items
  ...
  for (const string of arrOfStrs) {
    ...
    const [name, levelStr, itemsStr] = string.split(` / `);
 
      const level = Number(levelStr);
      const itemsArr = itemsStr ? itemsStr.split(`, `) : [];
 
      addToResultArr(name, level, itemsArr);
  }
