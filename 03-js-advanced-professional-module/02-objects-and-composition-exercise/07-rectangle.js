...
The rectangle needs to have a width (Number), height (Number), and color (String) properties, which are set via arguments during creation, 
...
function createRectangleObj(width, height, color) {
  ...
  const capitalLetter = color[0].toUpperCase();
  const editedColor = capitalLetter + color.slice(1);
